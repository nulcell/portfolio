...
understand a platform than to operate one end to end — networking, storage,
...
two-node etcd is strictly worse than one.
...
Everything else — cert-manager, Longhorn, monitoring, gateways, every
...
schedule. Nothing sensitive is in git at all — the SOPS setup is parked in
...
above to Alertmanager and — this is the fun part — to
**[Falco Talon](https://docs.falco-talon.org/)**, the response engine.
Talon can terminate or isolate a pod on a matching detection and record a
...
**Grafana Alloy** shipping pod logs via the Kubernetes API — no host volume
...
the times a dashboard beats `kubectl`.
...
would miss), the 3-node HA control plane and a 2.5GbE upgrade to go with it,
...
[nulcell/homecloud](https://github.com/nulcell/homecloud) — configs,
